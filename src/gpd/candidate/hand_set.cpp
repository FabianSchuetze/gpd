...
void HandSet::evalHands(const util::PointList &point_list,
    ...
    FingerHand finger_hand(hand_geometry_.finger_width_,
                           ...
                           hand_geometry_.depth_, num_finger_placements_);

    // Set the forward and lateral axis of the robot hand frame (closing
    ...
    for (int i = 0; i < angles_.rows(); i++) {
        ...
        if (finger_hand.getHand().any()) {
            ...
            if (deepen_hand_) {
                ...
                finger_idx = finger_hand.deepenHand(
                    point_list_cropped.getPoints(), hand_geometry_.init_bite_,
            ...
            modifyCandidate(*hands_[start + i], point_list_cropped,
                            indices_closing, finger_hand);
        }
    }
}
...
Eigen::Matrix3Xd HandSet::calculateShadow(const util::PointList &point_list,
    ...
    for (int i = 0; i < num_cams; i++) {
        if (camera_set(i) >= 1) {
            ...
            // Calculate the unit vector that points from the camera position to
            // the center of the point neighborhood.
            Eigen::Vector3d shadow_vec =
                center - point_list.getViewPoints().col(i);
    ...
    // Multiple camera view points: find the intersection of all sets of
    // occluded points.
...
void HandSet::labelHypothesis(const util::PointList &point_list,
    ...
    int label = antipodal_.evaluateGrasp(point_list, 0.003,
                                         finger_hand.getLateralAxis(),
...
Vector3iSet HandSet::intersection(const Vector3iSet &set1,
    ...
    for (Vector3iSet::const_iterator it = set1.begin(); it != set1.end();
         it++) {

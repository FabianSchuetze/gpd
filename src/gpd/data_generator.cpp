...
DataGenerator::DataGenerator(const std::string &config_filename) {
    ...
    Eigen::VectorXi::Map(&all_cam_sources_[0], cam_sources.rows()) =
        cam_sources;
}
...
void DataGenerator::generateData() {
    ...
    for (int i = 0; i < num_objects; i++) {
        printf("===> Generating images for object %d/%d: %s\n", i + 1,
               num_objects, objects[i].c_str());
        double t0 = omp_get_wtime();

        // Load mesh for ground truth.
        std::string prefix = data_root_ + objects[i];
        util::Cloud mesh =
            loadMesh(prefix + "_gt.pcd", prefix + "_gt_normals.csv");
        ...
        for (int j = 0; j < num_views_per_object_; j++) {
            ...
            std::vector<std::unique_ptr<candidate::Hand>> labeled_grasps_view(
                0);
            ...
            while (positives_view.size() < min_grasps_per_view_) {
                ...
                bool has_grasps =
                    detector_->createGraspImages(cloud, grasps, images);

                if (plot_grasps) {
                    // Plot plotter;
                    //        plotter.plotNormals(cloud.getCloudOriginal(),
                    //        cloud.getNormals());
                    //        plotter.plotFingers(grasps,
                    //        cloud.getCloudOriginal(), "Grasps on view");
                ...
                std::vector<int> labels =
                    detector_->evalGroundTruth(mesh, grasps);
                ...
                labeled_grasps_view.insert(
                    labeled_grasps_view.end(),
            ...
            balanceInstances(max_grasps_per_view_, positives_view,
                             negatives_view, positives_list, negatives_list);
        ...
        if ((i + 1) % store_step == 0) {
            ...
            test_offset =
                insertIntoHDF5(test_file_path, test_data, test_offset);
        ...
        printf(
            "Estimated time remaining (based on time for this object): %4.4fh "
            "or "
            "%4.4fs.\n",
    ...
    reshapeHDF5(train_file_path, train_file_path_resized, "images",
                train_offset, chunk_size_, max_in_memory_);
    reshapeHDF5(train_file_path, train_file_path_resized, "labels",
                train_offset, chunk_size_, max_in_memory_);
...
void DataGenerator::reshapeHDF5(const std::string &in, const std::string &out,
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if (!mat.isContinuous()) {
            std::cout
                << "Error: matrix is not continuous! Consider resizing using "
                   "python.\n";
            break;
        }
...
void DataGenerator::balanceInstances(int max_grasps_per_view,
    ...
    if (negatives_in.size() > positives_in.size()) {
        end = std::min(positives_in.size(),
                       (size_t)floor(0.5 * max_grasps_per_view));
...
void DataGenerator::storeHDF5(const std::vector<Instance> &dataset,
    ...
    if (!h5io->hlexists(IMAGE_DS_NAME)) {
        int n_dims_labels = 2;
        int dsdims_labels[n_dims_labels] = {static_cast<int>(dataset.size()),
                                            1};
        printf("Creating dataset <labels> ...\n");
        h5io->dscreate(n_dims_labels, dsdims_labels, CV_8UC1, LABELS_DS_NAME,
                       9);
        ...
        h5io->dscreate(n_dims, dsdims_images,
                       CV_8UC(dataset[0].image_->channels()), IMAGE_DS_NAME, 9,
                       chunks);
...
Eigen::Matrix4f DataGenerator::calculateTransform(const std::string &object,
    ...
    std::string cam_from_ref_key = "H_NP" + std::to_string(camera) +
                                   "_from_NP" +

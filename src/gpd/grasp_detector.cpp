...
GraspDetector::GraspDetector(const std::string &config_filename) {
    ...
    generator_params.workspace_ = config_file.getValueOfKeyAsStdVectorDouble(
    ...
    //  view_points << camera_position[0], camera_position[1],
    //  camera_position[2];
    ...
    std::string model_file =
        config_file.getValueOfKeyAsString("model_file", "");
    ...
    if (!model_file.empty() || !weights_file.empty()) {
        ...
        classifier_ = net::Classifier::create(
            ...
            static_cast<net::Classifier::Device>(device), batch_size);
    ...
    // Read grasp filtering parameters based on robot workspace and gripper
    // width.
    ...
    // Create plotter.
    plotter_ =
        std::make_unique<util::Plot>(hand_search_params.hand_axes_.size(),
                                     hand_search_params.num_orientations_);
}
...
std::vector<std::unique_ptr<candidate::Hand>> GraspDetector::detectGrasps(
    ...
    if (plot_samples_) {
        if (cloud.getSamples().cols() > 0) {
            plotter_->plotSamples(cloud.getSamples(),
    ...
    if (plot_filtered_candidates_) {
        plotter_->plotFingers3D(
            hand_set_list_filtered, cloud.getCloudOriginal(),
            "Filtered Grasps (Aperture, Workspace)", hand_geom);
    }
    if (filter_approach_direction_) {
        hand_set_list_filtered = filterGraspsDirection(hand_set_list_filtered,
                                                       direction_, thresh_rad_);
        if (plot_filtered_candidates_) {
            plotter_->plotFingers3D(hand_set_list_filtered,
                                    cloud.getCloudOriginal(),
    ...
    image_generator_->createImages(cloud, hand_set_list_filtered, images,
                                   hands);
    ...
    if (cluster_grasps_) {
        ...
        if (clusters.size() <= 3) {
            printf(
                "Not enough clusters found! Adding all grasps from previous "
                "step.");
            for (int i = 0; i < hands.size(); i++) {
...
GraspDetector::filterGraspsWorkspace(
    ...
    for (int i = 0; i < hand_set_list.size(); i++) {
        ...
        if (is_valid.any()) {
            ...
            hand_set_list_out[hand_set_list_out.size() - 1]->setIsValid(
                is_valid);
        }
    }

    printf(
        "Number of grasp candidates within workspace and gripper width: %d\n",
        remaining);
...
GraspDetector::filterGraspsDirection(
    ...
    for (int i = 0; i < hand_set_list.size(); i++) {
        ...
        for (int j = 0; j < hands.size(); j++) {
            if (is_valid(j)) {
                double angle =
                    acos(direction.transpose() * hands[j]->getApproach());
                if (angle > thresh_rad) {
                    is_valid(j) = false;
                } else {
                    remaining++;
